...
def get_language_specific_instructions(language: str) -> str:
    """获取特定语言的指令说明。"""
    instructions = {
        "java": "关注Servlet API、网络请求处理、文件读取等入口点。默认包含CodeQL官方的所有远程输入点Source（RemoteFlowSource）",
...
3. **对于Java项目，默认包含CodeQL官方定义的所有远程输入点（RemoteFlowSource），包括但不限于：**
   - HttpServletRequest 的各种方法（getParameter、getHeader、getCookies等）
   - Spring框架的 @RequestMapping、@GetMapping、@PostMapping 等注解的方法参数
   - JAX-RS 的 @Path、@GET、@POST 等注解的方法参数
   - Apache Struts 的 Action 方法参数
   - 其他Web框架的请求处理方法参数
...
4. **对于Java项目，默认包含CodeQL官方定义的所有远程输入点（RemoteFlowSource）并且加上路由参数注解**
