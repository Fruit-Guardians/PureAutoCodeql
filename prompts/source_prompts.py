...
{{
  ...
  "isAdditionalFlowStep": [
    {{
      "file_path": "相对路径",
      "function_name": "函数/方法名",
      "reason": "为何此处需要 isAdditionalFlowStep（如字符串转对象、反序列化等）",
      "flow_type": "类型转换|反序列化|反射调用|动态代理|集合操作"
    }}
  ]
}}

规则：
- 若没有发现候选函数，请输出：{{"candidates": [], "isAdditionalFlowStep": []}}
- **必要时可以使用 server-filesystem 读取文件内容进行补充验证**。
- 请确保输出为合法可解析的 JSON。
- 结果应与源码实际位置一致。
- **当发现数据流在类型转换、序列化/反序列化、反射等操作中可能中断时，应在 isAdditionalFlowStep 字段中标注**
