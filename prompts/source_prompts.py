...
2. 从 Source 到 Sink 的完整调用路径数据（包含中间转换点）
...
输出要求（必须严格遵守）：
- 仅输出 JSON（不要输出除 JSON 以外的任何文字、Markdown 或代码块标记）。
- JSON 结构包含两个独立部分：`candidates`（源点候选项）和 `source_to_sink_paths`（路径数据）
- 完整 JSON 结构如下：
{{
  "cve": "",
...
- 若没有发现候选函数，请输出：{{"candidates": [], "source_to_sink_paths": []}}
- **`candidates` 和 `source_to_sink_paths` 是两个独立的数据结构，互不干扰**
- `source_to_sink_paths` 专门用于后续的路径分析，不影响现有的源点选择逻辑
- 必要时可以使用 server-filesystem 读取文件内容进行补充验证
- 请确保输出为合法可解析的 JSON
- 结果应与 Sink 点位置和漏洞类型一致
- 使用 `server-filesystem` 的 `read_text_file` 时，不要同时设置 `head` 与 `tail`；如需同时查看文件顶部与底部，请分两次分别读取并标注来源。
- **路径数据必须包含完整的函数调用链、所有赋值操作和关键转换点**
