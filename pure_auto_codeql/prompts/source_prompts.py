...
任务目标：基于提供的 CVE 信息、Sink 点分析结果和 {language_upper} 文件路径，产出以下两部分内容：
1. 可能存在 Source 点的函数列表
{path_analysis_goal}
...
{path_analysis_action}
4. 基于 Sink 点的位置和类型，反向追踪可能的 Source 点
5. 关注与 Sink 点相关的数据流路径，查找可能的输入源
6. 结合 CVE 分析结果，理解漏洞的触发条件和输入来源
7. 给出每个候选 Source 点的理由与置信度（high/medium/low）
8. **使用LSP工具精确追踪，避免盲目搜索**

输出要求（必须严格遵守）：
- 仅输出 JSON（不要输出除 JSON 以外的任何文字、Markdown 或代码块标记）。
{path_analysis_json_desc}
- 完整 JSON 结构如下：
{{
  "cve": "",
  ...
  ]{candidates_end}{path_analysis_json_structure}
}}
...
{path_analysis_rules_1}
- 必要时可以使用 server-filesystem 读取文件内容进行补充验证
- 请确保输出为合法可解析的 JSON
- 结果应与 Sink 点位置和漏洞类型一致
- 使用 `server-filesystem` 的 `read_text_file` 时，不要同时设置 `head` 与 `tail`；如需同时查看文件顶部与底部，请分两次分别读取并标注来源。
{path_analysis_rules_2}
...
def build_source_analysis_with_sink_prompt(
    ...
    # Java、Python、C/C++ 使用同一条结构化 Source→Sink 路径契约。
    if language.lower() in ["java", "python", "c", "cpp"]:
        path_analysis_goal = "2. 从 Source 到 Sink 的完整调用路径数据（包含中间转换点）"
        path_analysis_action = """3. **追踪完整调用链路径**：
   - 记录从 Source 到 Sink 的完整函数调用序列
   - 识别路径中的关键转换点：
     * 变量赋值操作（数据传递）
     * 类型转换/强制转换
     * 反序列化操作
     * 算术运算（可能导致溢出）
     * 指针/数组偏移操作
   - 记录每个转换点的文件位置、行号和代码片段"""
        path_analysis_json_desc = "- JSON 结构包含两个独立部分：`candidates`（源点候选项）和 `source_to_sink_paths`（路径数据）"
        candidates_end = ","
        path_analysis_json_structure = """
    ...
    {{
      ...
      "call_chain": [
        {{
          ...
          "file_path": "文件路径",
          "line_number": 行号,
          "call_type": "direct_call|method_call|callback"
        }}
      ],
      "transformations": [
        {{
          "type": "assignment|type_conversion|deserialization|arithmetic|offset",
          "description": "转换描述",
          "file_path": "文件路径",
          "line_number": 行号,
          "code_snippet": "相关代码片段",
          "variable_name": "变量名（如适用）"
        }}
      ]
    }}
  ]"""
        path_analysis_rules_1 = """- **`candidates` 和 `source_to_sink_paths` 是两个独立的数据结构，互不干扰**
- `source_to_sink_paths` 专门用于后续的路径分析，不影响现有的源点选择逻辑"""
    ...
    return SOURCE_ANALYSIS_WITH_SINK_PROMPT.format(
        ...
        current_dir=current_dir,
        file_extension=file_extension,
        path_analysis_goal=path_analysis_goal,
        path_analysis_action=path_analysis_action,
        path_analysis_json_desc=path_analysis_json_desc,
        candidates_end=candidates_end,
        path_analysis_json_structure=path_analysis_json_structure,
        path_analysis_rules_1=path_analysis_rules_1,
        path_analysis_rules_2=path_analysis_rules_2,
        empty_result_instruction=empty_result_instruction
    )
