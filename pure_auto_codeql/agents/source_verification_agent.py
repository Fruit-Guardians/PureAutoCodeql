...
class SourceVerificationAgent:
    ...
    async def verify_analysis_result(
        ...
        """
        验证整个 Source 分析结果是否有效（通过生成查询并检查是否返回结果）。

        Args:
            analysis_result: 整个 Source 分析结果（JSON 字符串）
            max_retries: 最大重试次数
            timeout: 超时时间（秒）
        ...
        """
        _agent_name = agent_name or "Source Verification Agent"
        _agent_type = agent_type or "source_verification"
        
        logger.info(f"开始验证 Source 分析结果（整体验证）")
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"Source 验证尝试 {attempt + 1}/{max_retries}")
                ...
                task_id = f"source_verification_{hash(analysis_result)}"
                ...
                if is_empty:
                    # 空结果 = Source 分析无效（可能是幻觉）
                    logger.warning(f"Source 分析验证失败：查询返回空结果 (尝试 {attempt + 1}/{max_retries})")
                    # 记录失败的查询，供下一轮参考
                    failed_queries.append({
                        "attempt": attempt + 1,
                        "query": verification_ql,
                        "reason": "查询返回空结果（未找到任何匹配的 Source 点）"
                    })
                    continue
                else:
                    # 非空结果 = Source 分析有效
                    logger.info(f"✅ Source 分析验证通过：查询返回了结果")
        ...
        # 所有重试失败
        error_msg = f"Source 分析验证失败：已尝试 {max_retries} 次，查询始终返回空结果"
        logger.error(f"❌ {error_msg}")
        logger.info(f"将直接使用原始 Source 分析报告，不进行验证过滤")
    ...
    def _build_requirement(self, analysis_result: str) -> str:
        """
        基于整个 Source 分析结果构建验证需求。

        Args:
            analysis_result: Source 分析结果（JSON 字符串）

        Returns:
            自然语言需求描述
        """
        # 简化需求：验证分析结果中的 Source 点是否真实存在
        return f"""基于以下 Source 分析结果，生成一个 CodeQL 查询来验证这些 Source 点是否真实存在于代码库中。

Source 分析结果：
{analysis_result}

请生成一个简单的 CodeQL 查询，匹配分析结果中提到的 Source 点（函数、方法等）。查询应该能够返回至少一个结果，以证明这些 Source 点确实存在。"""
    ...
    def _load_verification_template(self) -> str:
        ...
        Raises:
            ...
            ValueError: 如果语言不支持
        """
        template = load_verification_template(self.language, "source")
        if template is None:
    ...
    def _build_prompt(
        ...
        if failed_queries:
            ...
            for failed in failed_queries:
                ...
                failed_context += "\n```\n\n"
            failed_context += "**请注意**：\n"
            failed_context += "1. 检查函数名、参数名是否正确\n"
            failed_context += "2. 检查文件路径匹配模式\n"
            failed_context += "3. 考虑使用更宽泛的匹配条件\n"
            failed_context += "4. 确认 Source 点的定义是否准确（参数位置、节点类型等）\n"
