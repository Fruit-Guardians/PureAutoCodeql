## 1. Design
- [x] 1.1 Document the target CLI command tree and backward-compatible aliases.
- [x] 1.2 Define shared request/result/error objects for CLI and API workflows.
- [x] 1.3 Identify current behavior that must remain compatible.
- [x] 1.4 Decide the package namespace needed to avoid the `agents` top-level
...
- [x] 2.3 Add console entry points and subcommands while preserving existing
      top-level flags.
- [x] 2.4 Route API task creation through the same service-layer validation.
- [x] 2.5 Update API and README documentation.
